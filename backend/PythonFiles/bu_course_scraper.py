...
def main():
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for i, future in enumerate(futures, 1):
            ...
            print(f"Processed page {i}/112 - found {len(result)} Hub courses")
            time.sleep(0.5) 
    with open('bu_hub_courses.csv', 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['code', 'name'] + hub_requirements
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        for course in all_courses:
            writer.writerow(course)
    print(f"Scraping complete. Saved {len(all_courses)} Hub courses to bu_hub_courses.csv")
