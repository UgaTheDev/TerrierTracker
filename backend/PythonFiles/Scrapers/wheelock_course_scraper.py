...
def scrape_page(page_num):
    url = f"https://www.bu.edu/academics/wheelock/courses/{page_num}/"
    try:
...
def main():
    ...
    with open('wheelock_all_courses.csv', 'w', newline='', encoding='utf-8') as csvfile:
    ...
    with open('wheelock_hub_courses.csv', 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['code', 'name'] + hub_requirements
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
    ...
    print(f"\nScraping complete.")
    print(f"Saved {len(all_courses)} total courses to wheelock_all_courses.csv")
    print(f"Saved {len(hub_courses)} Hub courses to wheelock_hub_courses.csv")
