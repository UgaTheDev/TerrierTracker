...
def scrape_page(page_num):
    url = f"https://www.bu.edu/academics/cas/courses/{page_num}/"
    try:
...
def main():
    ...
    with open('cas_all_courses.csv', 'w', newline='', encoding='utf-8') as csvfile:
    ...
    with open('cas_hub_courses.csv', 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['code', 'name'] + hub_requirements
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
    ...
    print(f"\nScraping complete.")
    print(f"Saved {len(all_courses)} total courses to cas_all_courses.csv")
    print(f"Saved {len(hub_courses)} Hub courses to cas_hub_courses.csv")
